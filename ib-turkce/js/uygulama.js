  ...
  kartKap.innerHTML = eserler
    .map((es) => {
      ...
      <a class="kart" style="--k-renk:${e(es.renk)}" href="eser.html?e=${e(es.slug)}">
        ${es.gorsel ? `<span class="gorsel" style="background-image:url('${e(es.gorsel)}')" role="img" aria-label="${e(es.ad)} görseli"></span>` : `<span class="emoji" aria-hidden="true">${es.emoji || "📖"}</span>`}
        <h3>${e(es.ad)}</h3>
...
async function eserSayfa() {
  ...
  const bant = document.getElementById("eser-bant");
  if (bant && es.gorsel) {
    bant.hidden = false;
    bant.style.backgroundImage = `url('${es.gorsel}')`;
    bant.setAttribute("role", "img");
    bant.setAttribute("aria-label", `${es.ad} görseli`);
    document.getElementById("eser-baslik").classList.add("bantli");
  }
